...
contract Augur is Controlled, Extractable {
    event MarketCreated(address indexed universe, bytes32 indexed topic, address indexed marketCreator, address market, uint256 marketCreationFee, int256 minPrice, int256 maxPrice, IMarket.MarketType marketType, bytes32 description, string extraInfo);
    ...
    function logMarketCreated(uint256 _marketCreationFee, int256 _minPrice, int256 _maxPrice, IMarket.MarketType _marketType, bytes32 _topic, bytes32 _description, string _extraInfo, IUniverse _universe, address _market, address _marketCreator) public returns (bool) {
