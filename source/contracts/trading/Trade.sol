...
contract Trade is CashAutoConverter, ReentrancyGuard {
    ...
    function publicSell(IMarket _market, uint8 _outcome, uint256 _fxpAmount, uint256 _price, uint256 _tradeGroupId) external payable convertToAndFromCash onlyInGoodTimes nonReentrant returns (bytes32) {
        return trade(msg.sender, Order.TradeDirections.Short, _market, _outcome, _fxpAmount, _price, _tradeGroupId);
    }

    function publicTrade(Order.TradeDirections _direction, IMarket _market, uint8 _outcome, uint256 _fxpAmount, uint256 _price, uint256 _tradeGroupId) external payable convertToAndFromCash onlyInGoodTimes nonReentrant returns (bytes32) {
    ...
    function trade(address _sender, Order.TradeDirections _direction, IMarket _market, uint8 _outcome, uint256 _fxpAmount, uint256 _price, uint256 _tradeGroupId) internal returns (bytes32) {
        ...
        Order.OrderTypes _type = Order.getOrderTradingTypeFromMakerDirection(_direction);
    ...
    function fillBestOrder(address _sender, Order.TradeDirections _direction, IMarket _market, uint8 _outcome, uint256 _fxpAmount, uint256 _price, uint256 _tradeGroupId) internal returns (uint256 _bestFxpAmount) {
        ...
        Order.OrderTypes _type = Order.getOrderTradingTypeFromFillerDirection(_direction);
        ...
        while (_orderId != 0 && _bestFxpAmount > 0 && msg.gas >= MINIMUM_GAS_NEEDED) {
            ...
            if (_type == Order.OrderTypes.Bid ? _orderPrice >= _price : _orderPrice <= _price) {
