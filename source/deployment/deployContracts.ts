...
import * as EthHttpProvider from "ethjs-provider-http";
...
export const TEST_ACCOUNT_SECRET_KEYS = ["Augur1", "Augur2", "Augur3"];
...
export async function compileAndDeployContracts(): Promise<ContractDeployer> {
    ...
    const ethHttpProviderPort = (typeof process.env.ETHEREUM_PORT === "undefined") ? await getPort() : parseInt(process.env.ETHEREUM_PORT || "0");
    // If no Ethereum host has been specified, use TestRPC.
    if (typeof process.env.ETHEREUM_HOST === "undefined") {
        const testRpcOptions = await initializeTestRpcClientOptions(ethHttpProviderPort, TEST_ACCOUNT_SECRET_KEYS);
        const rpcClient = new RpcClient();
        await rpcClient.listen(ethHttpProviderPort, testRpcOptions);
    }
    const ethHttpProvider = new EthHttpProvider("http://localhost:" + ethHttpProviderPort);
    const ethQuery = new EthQuery(ethHttpProvider);
    ...
    const contractDeployer = new ContractDeployer(ethQuery, contractJson, fromAccount, GAS_AMOUNT);
