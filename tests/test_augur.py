...
def test_known_universe_child_creation_failure(augur):
    with raises(TransactionFailed):
        augur.createChildUniverse("", [0, 10000, 0])
...
def test_log_requires(augur, universe):
    ...
    with raises(TransactionFailed):
        augur.logInitialReportSubmitted(universe.address, universe.address, universe.address, 1, False, [0, 10000, 0])

    with raises(TransactionFailed):
        augur.logInitialReportSubmitted(augur.address, augur.address, augur.address, 1, False, [0, 10000, 0])
