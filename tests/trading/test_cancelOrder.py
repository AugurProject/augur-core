...
from utils import bytesToLong, longTo20Bytes, fix, unfix
...
def test_cancelBid(contractsFixture):
    ...
    orderID = makeOrder.publicMakeOrder(orderType, fxpAmount, fxpPrice, market.address, outcomeID, str(bytearray(20)), str(bytearray(20)), tradeGroupID, sender=tester.k1)
    ...
    assert(ordersFetcher.getOrder(orderID, orderType, market.address, outcomeID) == [0, 0, '0x0000000000000000000000000000000000000000', 0, 0, str(bytearray(20)), str(bytearray(20)), 0]), "Canceled order elements should all be zero"
...
def test_exceptions(contractsFixture):
    ...
    with raises(TransactionFailed):
        cancelOrder.publicCancelOrder(longTo20Bytes(0), orderType, market.address, outcomeID, sender=tester.k1)
    with raises(TransactionFailed):
        cancelOrder.publicCancelOrder(longTo20Bytes(1), orderType, market.address, outcomeID, sender=tester.k1)
    with raises(TransactionFailed):
