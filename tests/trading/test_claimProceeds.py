...
def test_redeem_shares_in_binary_market(contractsFixture):
    ...
    contractsFixture.state.block.timestamp = market.getAutomatedReportDisputeDueTimestamp() + 1
    # finalize the market
    assert market.tryFinalizeAutomatedReport()
    # set timestamp to 3 days later (waiting period)
    contractsFixture.state.block.timestamp += long(timedelta(days = 3, seconds = 1).total_seconds())
