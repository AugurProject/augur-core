...
def test_make_ask_with_shares_take_with_shares(contractsFixture):
    ...
    makerFee = fix(completeSetFees, '0.6') / fix('1')
    takerFee = fix(completeSetFees, '0.4') / fix('1')
...
def test_make_bid_with_shares_take_with_shares(contractsFixture):
    ...
    makerFee = fix(completeSetFees, '0.4') / fix('1')
    takerFee = fix(completeSetFees, '0.6') / fix('1')
    assert leftoverInOrder == 0
    assert cash.balanceOf(tester.a1) == fix('1.2', '0.4') - makerFee
    assert cash.balanceOf(tester.a2) == fix('1.2', '0.6') - takerFee
    assert yesShareToken.balanceOf(tester.a1) == fix('1.2')
