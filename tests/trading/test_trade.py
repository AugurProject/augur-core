...
def test_one_bid_on_books_buy_full_order(contractsFixture, cash, market, universe):
    ...
    assert logs[1]["numCreatorTokens"] == fix('2', '0.6')
    assert logs[1]["numFillerShares"] == 0
    assert logs[1]["numFillerTokens"] == fix('2', '0.4')
...
def test_one_bid_on_books_buy_excess_order(contractsFixture, cash, market, universe):
    ...
    assert logs[1]["filler"] == bytesToHexString(tester.a2)
    assert logs[1]["numCreatorShares"] == 0
    assert logs[1]["numCreatorTokens"] == fix('4', '0.6')
    assert logs[1]["numFillerShares"] == 0
    assert logs[1]["numFillerTokens"] == fix('4', '0.4')
    assert logs[1]["price"] == fix('0.6')
    assert logs[1]["settlementFees"] == 0
    assert logs[1]["shareToken"] == market.getShareToken(YES)
    assert logs[1]["tradeGroupId"] == 42

    assert logs[2]["_event_type"] == "OrderCreated"
    assert logs[2]["amount"] == 1
    assert logs[2]["creator"] == bytesToHexString(tester.a2)
    assert logs[2]["numSharesEscrowed"] == 0
    assert logs[2]["numTokensEscrowed"] == fix('1', '0.4')
    assert logs[2]["orderId"] == fillOrderID
    assert logs[2]["price"] == fix('0.6')
...
def test_two_bids_on_books_buy_both(contractsFixture, cash, market, universe):
    ...
    assert logs[2]["_event_type"] == "BuyCompleteSets"
    assert logs[2]["sender"] == fillOrder.address
    assert logs[2]["amount"] == 1
    assert logs[2]["numOutcomes"] == 2
    assert logs[2]["market"] == market.address

    assert logs[3]["_event_type"] == "OrderFilled"
    assert logs[3]["creator"] == bytesToHexString(tester.a3)
    assert logs[3]["filler"] == bytesToHexString(tester.a2)
    assert logs[3]["numCreatorShares"] == 0
    assert logs[3]["numCreatorTokens"] == fix('1', '0.6')
    assert logs[3]["numFillerShares"] == 0
    assert logs[3]["numFillerTokens"] == fix('1', '0.4')
    assert logs[3]["price"] == fix('0.6')
    assert logs[3]["settlementFees"] == 0
    assert logs[3]["shareToken"] == market.getShareToken(YES)
    assert logs[3]["tradeGroupId"] == 42
...
def test_two_bids_on_books_buy_full_and_partial(contractsFixture, cash, market, universe):
    ...
    assert logs[1]["filler"] == bytesToHexString(tester.a2)
    assert logs[1]["numCreatorShares"] == 0
    assert logs[1]["numCreatorTokens"] == fix('12', '0.6')
    assert logs[1]["numFillerShares"] == 0
    assert logs[1]["numFillerTokens"] == fix('12', '0.4')
    assert logs[1]["price"] == fix('0.6')
    assert logs[1]["settlementFees"] == 0
    assert logs[1]["shareToken"] == market.getShareToken(YES)
    assert logs[1]["tradeGroupId"] == 42
    ...
    assert logs[3]["filler"] == bytesToHexString(tester.a2)
    assert logs[3]["numCreatorShares"] == 0
    assert logs[3]["numCreatorTokens"] == fix('3', '0.6')
    assert logs[3]["numFillerShares"] == 0
    assert logs[3]["numFillerTokens"] == fix('3', '0.4')
    assert logs[3]["price"] == fix('0.6')
    assert logs[3]["settlementFees"] == 0
    assert logs[3]["shareToken"] == market.getShareToken(YES)
    assert logs[3]["tradeGroupId"] == 42
...
def test_two_bids_on_books_buy_one_full_then_create(contractsFixture, cash, market, universe):
    ...
    assert logs[2]["creator"] == bytesToHexString(tester.a2)
    assert logs[2]["numSharesEscrowed"] == 0
    assert logs[2]["numTokensEscrowed"] == fix('3', '0.4')
    assert logs[2]["orderId"] == fillOrderID
    assert logs[2]["price"] == fix('0.6')
    assert logs[2]["shareToken"] == market.getShareToken(YES)
    assert logs[2]["tradeGroupId"] == 42
...
def test_one_ask_on_books_buy_full_order(contractsFixture, cash, market, universe):
    ...
    assert logs[1]["filler"] == bytesToHexString(tester.a2)
    assert logs[1]["numCreatorShares"] == 0
    assert logs[1]["numCreatorTokens"] == fix('12', '0.4')
    assert logs[1]["numFillerShares"] == 0
    assert logs[1]["numFillerTokens"] == fix('12', '0.6')
    assert logs[1]["price"] == fix('0.6')
    assert logs[1]["settlementFees"] == 0
    assert logs[1]["shareToken"] == market.getShareToken(YES)
    assert logs[1]["tradeGroupId"] == 42
...
def test_one_ask_on_books_buy_partial_order(contractsFixture, cash, market, universe):
    ...
    assert logs[1]["filler"] == bytesToHexString(tester.a2)
    assert logs[1]["numCreatorShares"] == 0
    assert logs[1]["numCreatorTokens"] == fix('7', '0.4')
    assert logs[1]["numFillerShares"] == 0
    assert logs[1]["numFillerTokens"] == fix('7', '0.6')
    assert logs[1]["price"] == fix('0.6')
    assert logs[1]["settlementFees"] == 0
    assert logs[1]["shareToken"] == market.getShareToken(YES)
    assert logs[1]["tradeGroupId"] == 42
...
def test_one_ask_on_books_buy_excess_order(contractsFixture, cash, market, universe):
    ...
    assert logs[2]["creator"] == bytesToHexString(tester.a2)
    assert logs[2]["numSharesEscrowed"] == 0
    assert logs[2]["numTokensEscrowed"] == fix('3', '0.6')
    assert logs[2]["orderId"] == fillOrderID
    assert logs[2]["price"] == fix('0.6')
    assert logs[2]["shareToken"] == market.getShareToken(YES)
    assert logs[2]["tradeGroupId"] == 42
...
def test_two_asks_on_books_buy_both(contractsFixture, cash, market, universe):
    ...
    assert logs[3]["filler"] == bytesToHexString(tester.a2)
    assert logs[3]["numCreatorShares"] == 0
    assert logs[3]["numCreatorTokens"] == fix('3', '0.4')
    assert logs[3]["numFillerShares"] == 0
    assert logs[3]["numFillerTokens"] == fix('3', '0.6')
    assert logs[3]["price"] == fix('0.6')
    assert logs[3]["settlementFees"] == 0
    assert logs[3]["shareToken"] == market.getShareToken(YES)
    assert logs[3]["tradeGroupId"] == 42
