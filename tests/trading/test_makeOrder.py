...
def test_publicMakeOrder_bid(contractsFixture):
    ...
    orderID = makeOrder.makeOrder(BID, 10**18, 10**17, market.address, 1, longTo32Bytes(0), longTo32Bytes(0), 7)
    assert orderID
...
def test_publicMakeOrder_ask(contractsFixture):
    ...
    orderID = makeOrder.makeOrder(ASK, 10**18, 10**17, market.address, 0, longTo32Bytes(0), longTo32Bytes(0), 7)
...
def test_publicMakeOrder_bid2(contractsFixture):
    ...
    orderID = makeOrder.makeOrder(orderType, fxpAmount, fxpPrice, market.address, outcome, longTo32Bytes(0), longTo32Bytes(0), tradeGroupID, sender=tester.k1)
...
def test_makeOrder_failure(contractsFixture):
    ...
    with raises(TransactionFailed):
        makeOrder.makeOrder(3, fix('1'), fix('0.6'), market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)

    # placeBid exceptions
    with raises(TransactionFailed):
        makeOrder.makeOrder(BID, fix('1'), fix('3'), market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)
    with raises(TransactionFailed):
        makeOrder.makeOrder(BID, 1, fix('0.6'), market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)

    # placeAsk exceptions
    with raises(TransactionFailed):
        makeOrder.makeOrder(ASK, fix('1'), 1, market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)
    with raises(TransactionFailed):
        makeOrder.makeOrder(ASK, 1, fix('0.6'), market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)
    ...
    with raises(TransactionFailed):
        makeOrder.makeOrder(ASK, fix('1'), fix('3'), market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)
    with raises(TransactionFailed):
        makeOrder.makeOrder(ASK, 1, fix('0.6'), market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)
    ...
    with raises(TransactionFailed):
        makeOrder.makeOrder(ASK, fix('1'), fix('0.6'), tester.a1, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)

    assert makeOrder.makeOrder(ASK, fix('1'), fix('0.6'), market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1) != 0, "Order ID should be non-zero"

    # makeOrder exceptions (post-placeBid/Ask)
    with raises(TransactionFailed):
        makeOrder.makeOrder(ASK, fix('1'), fix('0.6'), market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)
...
def test_ask_withPartialShares(contractsFixture):
    ...
    captureFilteredLogs(contractsFixture.chain.head_state, orders, logs)
    orderID = makeOrder.makeOrder(ASK, fix('2'), fix('0.6'), market.address, YES, longTo32Bytes(0), longTo32Bytes(0), 42, sender=tester.k1)
    assert cash.balanceOf(tester.a1) == fix('0')
