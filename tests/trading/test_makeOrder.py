...
from utils import bytesToLong, longTo20Bytes, longToHexString, bytesToHexString, fix, unfix, captureFilteredLogs
...
def test_publicMakeOrder_bid(contractsFixture):
    ...
    orderID = makeOrder.publicMakeOrder(BID, 10**18, 10**17, market.address, 1, longTo20Bytes(0), longTo20Bytes(0), 7)
    assert orderID
    ...
    assert order[BETTER_ORDER_ID] == bytearray(20)
    assert order[WORSE_ORDER_ID] == bytearray(20)
...
def test_publicMakeOrder_ask(contractsFixture):
    ...
    orderID = makeOrder.publicMakeOrder(ASK, 10**18, 10**17, market.address, 0, longTo20Bytes(0), longTo20Bytes(0), 7)
...
def test_publicMakeOrder_bid2(contractsFixture):
    ...
    orderID = makeOrder.publicMakeOrder(orderType, fxpAmount, fxpPrice, market.address, outcome, longTo20Bytes(0), longTo20Bytes(0), tradeGroupID, sender=tester.k1)
    assert orderID != bytearray(20), "Order ID should be non-zero"
...
def test_makeOrder_failure(contractsFixture):
    ...
    with raises(TransactionFailed):
        makeOrder.makeOrder(tester.a1, ASK, fix('1'), fix('0.6'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)
    with raises(TransactionFailed):
    ...
    with raises(TransactionFailed):
        makeOrder.publicMakeOrder(3, fix('1'), fix('0.6'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)

    # placeBid exceptions
    with raises(TransactionFailed):
        makeOrder.publicMakeOrder(BID, fix('1'), fix('3'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)
    with raises(TransactionFailed):
        makeOrder.publicMakeOrder(BID, 1, fix('0.6'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)

    # placeAsk exceptions
    with raises(TransactionFailed):
        makeOrder.publicMakeOrder(ASK, fix('1'), 1, market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)
    with raises(TransactionFailed):
        makeOrder.publicMakeOrder(ASK, 1, fix('0.6'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)
    ...
    with raises(TransactionFailed):
        makeOrder.publicMakeOrder(ASK, fix('1'), fix('3'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)
    with raises(TransactionFailed):
        makeOrder.publicMakeOrder(ASK, 1, fix('0.6'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)
    ...
    with raises(TransactionFailed):
        makeOrder.publicMakeOrder(ASK, fix('1'), fix('0.6'), tester.a1, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)

    assert makeOrder.publicMakeOrder(ASK, fix('1'), fix('0.6'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1) != 0, "Order ID should be non-zero"

    # makeOrder exceptions (post-placeBid/Ask)
    with raises(TransactionFailed):
        makeOrder.publicMakeOrder(ASK, fix('1'), fix('0.6'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)
...
def test_ask_withPartialShares(contractsFixture):
    ...
    captureFilteredLogs(contractsFixture.chain.head_state, orders, logs)
    orderID = makeOrder.publicMakeOrder(ASK, fix('2'), fix('0.6'), market.address, YES, longTo20Bytes(0), longTo20Bytes(0), 42, sender=tester.k1)
    assert cash.balanceOf(tester.a1) == fix('0')
