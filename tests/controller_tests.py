...
# t.gas_limit = 4100000
s = t.state()
c = s.abi_contract('controller_test.se')


def test_setController():
	print("Testing setController")
	# set the controller = 0x0 now that we have it
	print "Setting controller to: %s" % binascii.hexlify(c.address)
	out = c.setController(binascii.hexlify(c.address))
	print "setController output: %s" % out
	assert(out == 1), "setController succeeded"
	# Test a failure
	try:
		raise Exception(c.setController(binascii.hexlify(c.address)))
...
def test_whitelisting():
	...
	try:
		raise Exception(c.assertIsWhitelisted(t.a2))
	except Exception as exc:
...
def test_suicideFunds():
	...
	# Test with another non-controller address  (redundant?)
	try:
		raise Exception(c.suicideFunds(t.a2, sender=t.k2))
	except Exception as exc:
		assert(isinstance(exc, ethereum.tester.TransactionFailed)), "suicideFunds should fail when attempted by non-controller address"
	# Test with controller address
	out = c.suicideFunds(t.a0, sender=t.k0)
...
def test_Ownership():
	...
	out = c.transferOwnership(t.k1, t.a1, 0, 0)
	...
	out = c.transferOwnership(t.k0, t.a0, 0, 0, sender=t.k1)
	print "transferOwnership output: %s" % out
...
def test_getMode():
	print("Testing getMode")   # combine this section into one of the others?
	out = c.getMode()
	print "getMode output: %s" % out
	assert(out == 0), "getMode should be dev"
