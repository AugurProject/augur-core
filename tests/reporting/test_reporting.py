...
def test_reportingFullHappyPath(getStakeBonus, localFixture, universe, cash, market):
    ...
    stakeTokenNo = localFixture.getOrCreateStakeToken(market, [10**4,0])
    stakeTokenYes = localFixture.getOrCreateStakeToken(market, [0,10**4])
    ...
    noUniverse = localFixture.getOrCreateChildUniverse(universe, market, [10**4,0])
    ...
    yesUniverse = localFixture.getOrCreateChildUniverse(universe, market, [0,10**4])
...
def test_designatedReportingHappyPath(localFixture, universe, market):
    ...
    proceedToDesignatedReporting(localFixture, universe, market, [0,10**4])
    ...
    logs = []
    captureFilteredLogs(localFixture.chain.head_state, localFixture.contracts['Augur'], logs)
    assert localFixture.designatedReport(market, [0,10**4], tester.k0)
    ...
    assert logs[3]['stakeToken'] == localFixture.getOrCreateStakeToken(market, [0,10**4]).address
    assert logs[3]['market'] == market.address
    assert logs[3]['payoutNumerators'] == [0,10**4]
...
def test_firstReportingHappyPath(makeReport, localFixture, universe, market):
    ...
    proceedToFirstReporting(localFixture, universe, market, makeReport, 1, [0,10**4], [10**4,0])
    ...
    assert log['stakeToken'] == localFixture.getOrCreateStakeToken(market, [0,10**4]).address
    assert log['market'] == market.address
    assert log['payoutNumerators'] == [0,10**4]
...
def test_lastReportingHappyPath(localFixture, makeReport, universe, market, cash):
    ...
    proceedToFirstReporting(localFixture, universe, newMarket, makeReport, 1, [0,10**4], [10**4,0])
    newMarketStakeToken = localFixture.getOrCreateStakeToken(newMarket, [0,10**4])
    ...
    proceedToLastReporting(localFixture, universe, market, makeReport, 1, 3, [0,10**4], [10**4,0], 2, [10**4,0], [0,10**4])
...
def test_forking(localFixture, makeReport, finalizeByMigration, universe, market):
    ...
    proceedToForking(localFixture, universe, market, makeReport, 1, 3, 3, [0,10**4], [10**4,0], 2, [10**4,0], [0,10**4], [10**4,0])

    # Finalize the market
    finalizeForkingMarket(localFixture, universe, market, finalizeByMigration, tester.a1, tester.k1, tester.a0, tester.k0, tester.a2, tester.k2, [0,10**4], [10**4,0])
...
def test_forkMigration(localFixture, makeReport, finalizeByMigration, universe, cash, market):
    ...
    proceedToDesignatedReporting(localFixture, universe, newMarket, [0,10**4])
    if (makeReport):
        localFixture.designatedReport(newMarket, [0,10**4], tester.k0)

    # We proceed the standard market to the FORKING state
    proceedToForking(localFixture, universe, market, makeReport, 1, 2, 3, [0,10**4], [10**4,0], 2, [10**4,0], [0,10**4], [10**4,0])
...
def test_noReports(localFixture, pastDisputePhase, universe, market):
    # Proceed to the FIRST REPORTING phase
    proceedToFirstReporting(localFixture, universe, market, False, 1, [0,10**4], [10**4,0])
    ...
    stakeToken = localFixture.getOrCreateStakeToken(market, [0,10**4])
...
def test_invalid_first_report(localFixture, universe, cash, market):
    ...
    stakeTokenInvalid = localFixture.getOrCreateStakeToken(market, [long(0.5 * 10 ** 4), long(0.5 * 10 ** 4)], True)
...
def test_invalid_designated_report(localFixture, universe, cash, market):
    ...
    initialMarketCreatorETHBalance = localFixture.chain.head_state.get_balance(market.getMarketCreatorMailbox())
    assert localFixture.designatedReport(market, [long(0.5 * 10 ** 4), long(0.5 * 10 ** 4)], tester.k0, True)
...
def test_cannot_fork_twice(localFixture, universe, cash, market):
    ...
    proceedToLastReporting(localFixture, universe, market, True, 1, 3, [0,10**4], [10**4,0], 2, [10**4,0], [0,10**4])
    proceedToLastReporting(localFixture, universe, newMarket, True, 1, 3, [0,10**4], [10**4,0], 2, [10**4,0], [0,10**4])
