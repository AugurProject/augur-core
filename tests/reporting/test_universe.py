...
def test_get_reporting_window(contractsFixture):
    universe = contractsFixture.createUniverse(1, "10")
    timestamp = contractsFixture.chain.head_state.timestamp
    duration =  contractsFixture.constants.REPORTING_DURATION_SECONDS()
    dispute_duration = contractsFixture.constants.REPORTING_DISPUTE_DURATION_SECONDS()
    ...
    end_timestamp = contractsFixture.chain.head_state.timestamp + 1
    ...
    end_timestamp_des_test = end_timestamp + contractsFixture.constants.DESIGNATED_REPORTING_DURATION_SECONDS() + contractsFixture.constants.DESIGNATED_REPORTING_DISPUTE_DURATION_SECONDS() + 1 + total_dispute_duration
    ...
    assert universe.getPreviousReportingWindow() == universe.getReportingWindowByTimestamp(contractsFixture.chain.head_state.timestamp - total_dispute_duration)
    assert universe.getCurrentReportingWindow() == universe.getReportingWindowByTimestamp(contractsFixture.chain.head_state.timestamp)
    assert universe.getNextReportingWindow() == universe.getReportingWindowByTimestamp(contractsFixture.chain.head_state.timestamp + total_dispute_duration)
    
def test_create_child_universe(contractsFixture):
    universe = contractsFixture.createUniverse(2, "15")
    ...
    other_universe = contractsFixture.createUniverse(3, "1")
    other_child = other_universe.getOrCreateChildUniverse("21")
    assert universe.isParentOf(other_child) == False

def test_universe_contains_reporting_win(contractsFixture):
    universe = contractsFixture.createUniverse(0, "5")
    other_universe = contractsFixture.createUniverse(1, "15")
    other_curr_reporting_win = other_universe.getCurrentReportingWindow()
    reportingWindow = contractsFixture.applySignature('ReportingWindow', universe.getCurrentReportingWindow())
    ...
    nonReportingWindow = contractsFixture.upload('../source/contracts/reporting/ReportingToken.sol', 'nonReportingWindow')
    assert universe.isContainerForReportingWindow(nonReportingWindow.address) == False

    # create reporting windo with startTime of 0
    reporting_window_factory = contractsFixture.upload('../source/contracts/factories/ReportingWindowFactory.sol', 'reporting_window_factory')
    zero_window = reporting_window_factory.createReportingWindow(contractsFixture.controller.address, universe.address, 0)
    assert universe.isContainerForReportingWindow(zero_window) == False
    
def test_universe_contains_dispute_bond_token(contractsFixture):
    universe = contractsFixture.createUniverse(0, "55")
    # Pass in non Dispute Bond Token object address
    nonDisputeBondToken = contractsFixture.upload('../source/contracts/reporting/ReportingToken.sol', 'nonReportingWindow')
    ...
    dispute_bond_token_factory = contractsFixture.upload('../source/contracts/factories/DisputeBondTokenFactory.sol', 'dispute_bond_token_factory')
    bond = dispute_bond_token_factory.createDisputeBondToken(contractsFixture.controller.address, market.address, tester.a0, 10, "15")
    assert universe.isContainerForDisputeBondToken(bond) == False

    # Pass in bond with market address of 0
    zero_market_bond = dispute_bond_token_factory.createDisputeBondToken(contractsFixture.controller.address, 0, tester.a0, 10, "15")
...
def test_universe_contains_market(contractsFixture):
    universe = contractsFixture.createUniverse(1, "25")
    # market in different universe
    market = contractsFixture.binaryMarket
    assert universe.isContainerForMarket(market.address) == False
        
    uni_market = contractsFixture.createReasonableBinaryMarket(universe, contractsFixture.cash)
    assert universe.isContainerForMarket(uni_market.address)

def test_universe_reporting_token(contractsFixture):
    universe = contractsFixture.createUniverse(1, "25")
    reporting_token_factory = contractsFixture.upload('../source/contracts/factories/ReportingTokenFactory.sol', 'reporting_token_factory')

    # Reporting token not associated with this universe market
    different_reporting_token = reporting_token_factory.createReportingToken(contractsFixture.controller.address, contractsFixture.binaryMarket.address, [0, 10**18])
    ...
    uni_market = contractsFixture.createReasonableBinaryMarket(universe, contractsFixture.cash)
    good_reporting_token = contractsFixture.getReportingToken(uni_market, [0,10**18])
    assert universe.isContainerForMarket(uni_market.address)
    assert universe.isContainerForReportingToken(good_reporting_token.address)

def test_universe_contains_share_token(contractsFixture):
    universe = contractsFixture.createUniverse(1, "25")
    share_token_factory = contractsFixture.upload('../source/contracts/factories/ShareTokenFactory.sol', 'share_token_factory')

    # Share token not associated with this universe market
    different_share_token = share_token_factory.createShareToken(contractsFixture.controller.address, contractsFixture.binaryMarket.address, 1)
    ...
    # Create market and test it's reporting token
    uni_market = contractsFixture.createReasonableBinaryMarket(universe, contractsFixture.cash)
    good_share_token = contractsFixture.getShareToken(uni_market, 1)
