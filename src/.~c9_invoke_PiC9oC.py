...
def test_trading():
    ...
    while i < 50:
        ...
        while a < 3:
            ...
            print bal
            gas_use(s)
            assert(c.getCumScale(bin_market)==cumScale[0])
            ...
            bal = c.balance(t.k2)
            print bal
            assert(c.getParticipantSharesPurchased(bin_market, 1, 1)==11*2**64)
            assert(c.getParticipantSharesPurchased(bin_market, 1, 2)==9*2**64)
            bal = c.balance(s.block.coinbase)
            print bal
            assert(c.getParticipantSharesPurchased(bin_market, participantNumberIDK1, 1)==2**64)
            ...
            assert(c.getSharesValue(bin_market)==c.getCumScale(bin_market)*12)
