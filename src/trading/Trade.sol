...
contract Trade is Controlled, ReentrancyGuard {
    ...
    function publicBuy(IMarket _market, uint8 _outcome, uint256 _fxpAmount, uint256 _fxpPrice, uint256 _tradeGroupId) external onlyInGoodTimes nonReentrant returns (bytes32) {
        return trade(msg.sender, Order.TradeDirections.Long, _market, _outcome, _fxpAmount, _fxpPrice, _tradeGroupId);
    }

    function publicSell(IMarket _market, uint8 _outcome, uint256 _fxpAmount, uint256 _fxpPrice, uint256 _tradeGroupId) external onlyInGoodTimes nonReentrant returns (bytes32) {
        return trade(msg.sender, Order.TradeDirections.Short, _market, _outcome, _fxpAmount, _fxpPrice, _tradeGroupId);
    }

    function publicTrade(Order.TradeDirections _direction, IMarket _market, uint8 _outcome, uint256 _fxpAmount, uint256 _fxpPrice, uint256 _tradeGroupId) external onlyInGoodTimes nonReentrant returns (bytes32) {
        return trade(msg.sender, _direction, _market, _outcome, _fxpAmount, _fxpPrice, _tradeGroupId);
    }

    function publicTakeBestOrder(Order.TradeDirections _direction, IMarket _market, uint8 _outcome, uint256 _fxpAmount, uint256 _fxpPrice, uint256 _tradeGroupId) external onlyInGoodTimes nonReentrant returns (uint256) {
    ...
    function takeBestOrder(address _sender, Order.TradeDirections _direction, IMarket _market, uint8 _outcome, uint256 _fxpAmount, uint256 _fxpPrice, uint256 _tradeGroupId) internal returns (uint256 _bestFxpAmount) {
        ...
        while (_orderId != 0 && _bestFxpAmount > 0 && msg.gas >= MINIMUM_GAS_NEEDED) {
            uint256 _fxpOrderPrice = _orders.getPrice(_orderId);
