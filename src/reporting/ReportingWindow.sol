...
contract ReportingWindow is DelegationTarget, Typed, Initializable, IReportingWindow {
    ...
    function createNewMarket(uint256 _endTime, uint8 _numOutcomes, uint256 _payoutDenominator, uint256 _tickSize, uint256 _feePerEthInWei, ICash _denominationToken, address _creator, address _automatedReporterAddress, bytes32 _topic) public afterInitialized payable returns (IMarket _newMarket) {
        require(block.timestamp < startTime);
        require(branch.getReportingWindowByMarketEndTime(_endTime, _automatedReporterAddress != 0).getTypeName() == "ReportingWindow");
        _newMarket = MarketFactory(controller.lookup("MarketFactory")).createMarket.value(msg.value)(controller, this, _endTime, _numOutcomes, _payoutDenominator, _tickSize, _feePerEthInWei, _denominationToken, _creator, _automatedReporterAddress, _topic);
