...
contract MarketCreation {
    function createScalarMarket(IBranch _branch, uint256 _endTime, uint256 _feePerEthInWei, ICash _denominationToken, uint256 _numTicks, address _automatedReporterAddress, bytes32 _topic) payable public returns (IMarket) {
        IReportingWindow _reportingWindow = _branch.getReportingWindowByMarketEndTime(_endTime, (_automatedReporterAddress != 0));
        return _reportingWindow.createNewMarket.value(msg.value)(_endTime, 2, _numTicks, _feePerEthInWei, _denominationToken, msg.sender, _automatedReporterAddress, _topic);
    }
